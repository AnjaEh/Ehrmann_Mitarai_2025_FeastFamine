...
@author: Anja Ehrmann
Feast Famine Model (Parameter Screens) for Ehrmann & Mitarai (2026). Generates plots for figure 3, and supplementary figures S4
...
def simulation(initial_conditions, pars):
    ...
    return [t_final, phage_T4_final, phage_T7_final, bacteria_final, infected_T4_final,
            infected_T7_final, substrate_final, starvation_time, phage_T4_trend, phage_T7_trend, phage_T4_cycle1, phage_T7_cycle1, latency_delay, substrate_end]
...
pars['fresh_bacteria']  = 0.5 * S_0
pars['dilution'] = 0.1

pars['latency_T4'] = 0.8
pars['gamma_delay'] = 0.5
pars['S_zero'] = S_0
...
scan_parameter_1 = 'gamma delay'
list_parameter_1 = np.linspace(0.1, 2, 10)
#list_parameter_1 = np.linspace(0.35, 0.85, 12)
...
for j in list_parameter_2:
    ...
    for i in list_parameter_1:
        print(f'Screening gamma delay {i}')
        pars['gamma_delay'] = i
        #pars['fresh_bacteria']  = 0.02 * S_0
        ...
        t_final, phage_T4_final, phage_T7_final, bacteria_final, infected_T4_final, \
            infected_T7_final, substrate_final, starvation_time, phage_T4_trend, phage_T7_trend, \
                phage_T4_cycle1, phage_T7_cycle1, typical_latency_delay, substrate_end = output
        ...
        result_latency_T4_i = np.append(result_latency_T4_i, np.nanmedian(typical_latency_delay))
...
alpha = pars['fresh_bacteria'] / S_0
#alpha = 0.001
today = date.today().strftime('%Y%m%d')
filename = f"{today}_FeastFamine_GammaScreen_trend_diff_alpha_{alpha:.3g}.csv"
exportData(result_trend_diff, directory, f"{today}_FeastFamine_GammaScreen_trend_diff_alpha_{alpha:.3g}.csv")
exportData(result_starvation[1:,:], directory, f"{today}_FeastFamine_GammaScreen_StarvationTime_alpha_{alpha:.3g}.csv")
exportData(result_T4_cycle1[1:,:], directory, f"{today}_FeastFamine_GammaScreen_T4Cycle1_alpha_{alpha:.3g}.csv")
exportData(result_T7_cycle1[1:,:], directory, f"{today}_FeastFamine_GammaScreen_T7Cycle1_alpha_{alpha:.3g}.csv")
exportData(result_latency_T4[1:,:], directory, f"{today}_FeastFamine_GammaScreen_LatencyT4_alpha_{alpha:.3g}.csv")
exportData(result_substrate_end[1:,:], directory, f"{today}_FeastFamine_GammaScreen_SubstrateEnd_alpha_{alpha:.3g}.csv")
...
plt.savefig(f"Plots/{today}_FeastFamine_GammaScreen_trend_diff_alpha_{alpha:.3g}.svg", format = 'svg', dpi=600, bbox_inches='tight')
plt.show()
...
plt.savefig(f"Plots/{today}_FeastFamine_GammaScreen_T7Cycle1_alpha_{alpha:.3g}.svg", format = 'svg', dpi=600, bbox_inches='tight')
plt.show()
...
plt.savefig(f"Plots/{today}_FeastFamine_GammaScreen_T4Cycle1_alpha_{alpha:.3g}.svg", format = 'svg', dpi=600, bbox_inches='tight')
plt.show()
...
plt.savefig(f"Plots/{today}_FeastFamine_GammaScreen_starvation_time_alpha_{alpha:.3g}.svg", format = 'svg', dpi=600, bbox_inches='tight')
plt.show()
...
plt.savefig(f"Plots/{today}_FeastFamine_GammaScreen_LatencyT4_alpha_{alpha:.3g}.svg", format = 'svg', dpi=600, bbox_inches='tight')
plt.show()
...
fig, ax = plt.subplots()
im = ax.imshow(np.flipud(np.log10(result_substrate[1:,:])), interpolation = 'none', vmin = 2, vmax = 6, cmap = 'coolwarm')
fig.colorbar(im, ax=ax, shrink = 0.8)
ax.set_title('substrate')
plt.show()

fig, ax = plt.subplots()
im = ax.imshow(np.flipud(np.log10(result_substrate_end[1:,:])), interpolation = 'none', vmin = 2, vmax = 6, cmap = 'coolwarm')
fig.colorbar(im, ax=ax, shrink = 0.8)
ax.set_title('substrate end point')
plt.savefig(f"Plots/{today}_FeastFamine_GammaScreen_SubstrateEnd_alpha_{alpha:.3g}.svg", format = 'svg', dpi=600, bbox_inches='tight')
plt.show()