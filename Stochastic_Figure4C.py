# -*- coding: utf-8 -*-
"""Feast Famine model for Ehrmann & Mitarai (2026) with drawing random alpha values. Generates Figure 4C.
...
exportData(trend_diff, directory, f"{today}_FeastFamine_Stochastic_trend_diff_gammaDelay_{gamma_delay:.3g}.csv")
exportData(result_trend_T4, directory, f"{today}_FeastFamine_Stochastic_trend_T4_gammaDelay_{gamma_delay:.3g}.csv")
exportData(result_trend_T7, directory, f"{today}_FeastFamine_Stochastic_trend_T7_gammaDelay_{gamma_delay:.3g}.csv")


#%%
#Calculate row wise median of trend_diff
median_trend_diff = np.median(trend_diff[1:], axis = 1)
exportData(median_trend_diff, directory, f"{today}_FeastFamine_Stochastic_median_trend_diff_gammaDelay_{gamma_delay:.3g}.csv")
...
plt.savefig(f"Plots/{today}_FeastFamine_Stochastic_trend_diff_gammaDelay_{gamma_delay:.3g}.svg", format = 'svg', dpi=600, bbox_inches='tight')
plt.show()

from scipy.stats import gmean

alpha = np.logspace(-3, 0.3, num =20)
print('Geometric mean of alpha values: ' + str(np.exp(np.mean(np.log(alpha)))))
print('Geometric mean of alpha values (scipy function): ' + str(gmean(alpha)))