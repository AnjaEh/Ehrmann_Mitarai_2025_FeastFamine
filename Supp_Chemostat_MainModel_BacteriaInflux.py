"""Chemostat model for Ehrmann & Mitarai (2026) with bacteria influx. Generates plots for supplementary figure s3.
...
def system_of_equations(t,y, pars):
    ...
    #dP4_dt = burst_T4*infected_T4_5*n/latency_delay - adsorption*phage_T4*bacteria - p_deg * phage_T4 - flow_rate * phage_T4
    #dP7_dt = burst_T7 * infected_T7_5 *n/latency_T7 - adsorption * phage_T7 * bacteria - flow_rate * phage_T7 -p_deg * phage_T7
...
result_typical_latency = np.empty((len(burst_T7_stat), len(flow_rates)))
result_median_latency = np.empty((len(burst_T7_stat), len(flow_rates)))
...
for j in burst_T7_stat:
    ...
    for flow_rate in flow_rates:
        ...
        infected_T4 = sol.y[8] + sol.y[4] + sol.y[5] + sol.y[6] + sol.y[7]
        infected_T7 = sol.y[13] + sol.y[9] + sol.y[10] + sol.y[11] + sol.y[12]
        ...
        if np.any(valid_mask):
            weighted_mean_latency = np.nansum(latency_T4_window_cycle * delta_T_cycle) / np.sum(delta_T_cycle[valid_mask])
            median_latency = np.nanmedian(latency_T4_window_cycle)
        else:
...
print(directory)
gamma_delay = pars['gamma_delay']
alpha = pars['bacteria_in'] / pars['substrate_in']
today = date.today().strftime('%Y%m%d')
experiment_name = "BacteriaInflux"
exportData(result_trend_diff, directory, f"{today}_Chemostat_FlowScreen_{experiment_name}_trend_diff_Alpha_{alpha:.3g}.csv")
exportData(result_trend_norm_diff, directory, f"{today}_Chemostat_FlowScreen_{experiment_name}_trend_norm_diff_Alpha_{alpha:.3g}.csv")
exportData((result_starvation_cycles/(oscillation_count)), directory, f"{today}_Chemostat_FlowScreen_{experiment_name}_StarvationCycles_Alpha_{alpha:.3g}.csv")

exportData(result_typical_latency, directory, f"{today}_Chemostat_FlowScreen_{experiment_name}_LatencyT4_Alpha_{alpha:.3g}.csv")
exportData(result_median_latency, directory, f"{today}_Chemostat_FlowScreen_{experiment_name}_LatencyT4_Median_Alpha_{alpha:.3g}.csv")
exportData(result_substrate, directory, f"{today}_Chemostat_FlowScreen_{experiment_name}_SubstrateEnd_Alpha_{alpha:.3g}.csv")
...
plt.imshow(np.flipud(result_trend_norm_diff), extent = [(flow_rates[0]), (flow_rates[-1]), (burst_T7_stat[0]), burst_T7_stat[-1]], aspect = 'auto', cmap = 'coolwarm', vmin = -0.05, vmax = 0.05)
plt.colorbar()
plt.xlabel('flow rate')
plt.ylabel('burst size T7stat')
plt.title('Trend difference, normalized by flow rate')
plt.savefig(f"Plots/{today}_Chemostat_FlowScreen_{experiment_name}_trend_norm_diff_Alpha_{alpha:.3g}.svg", format = 'svg', dpi=600, bbox_inches='tight')
plt.show()
...
plt.xlabel('log10(flow rate)')
plt.ylabel('burst size T7stat')
plt.title('Number of oscillations in the first third of time series')
plt.show()
...
plt.savefig(f"Plots/{today}_Chemostat_FlowScreen_{experiment_name}_StarvationCycles_Alpha_{alpha:.3g}.svg", format = 'svg', dpi=600, bbox_inches='tight')
plt.show()


plt.imshow(np.flipud(result_typical_latency), extent = [np.log10(flow_rates[0]), np.log10(flow_rates[-1]), (burst_T7_stat[0]), burst_T7_stat[-1]], aspect = 'auto', vmin = 0, vmax = 10, cmap = 'RdYlBu')
plt.colorbar()
plt.xlabel('log10(flow rate)')
plt.ylabel('burst size T7stat')
plt.title('Typical latency time T4')

plt.savefig(f"Plots/{today}_Chemostat_FlowScreen_{experiment_name}_TypicalLatency_Scaled_Alpha_{alpha:.3g}.svg", format = 'svg', dpi=600, bbox_inches='tight')
plt.show()

plt.imshow(np.flipud(result_median_latency), extent = [np.log10(flow_rates[0]), np.log10(flow_rates[-1]), (burst_T7_stat[0]), burst_T7_stat[-1]], aspect = 'auto', vmin = 0, vmax = 100, cmap = 'RdYlBu')
plt.colorbar()
plt.xlabel('log10(flow rate)')
plt.ylabel('burst size T7stat')
plt.title('Typical latency time T4 Median')
#plt.savefig(f"Plots/{today}_Chemostat_FlowScreen_{experiment_name}_MedianLatency_Alpha_{alpha:.3g}.svg", format = 'svg', dpi=600, bbox_inches='tight')
plt.show()
