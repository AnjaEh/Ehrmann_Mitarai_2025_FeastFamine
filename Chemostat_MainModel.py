"""Chemostat model for Ehrmann & Mitarai (2025). Generates plots for figure 2, supplementary figure s1 and s2.
...
experiment_name = "CorrectedLatency_standard"

exportData(result_trend_diff, directory, f"{today}_Chemostat_FlowScreen_{experiment_name}_trend_diff_GammaDelay_{gamma_delay:.3g}.csv")
exportData(result_trend_norm_diff, directory, f"{today}_Chemostat_FlowScreen_{experiment_name}_trend_norm_diff_GammaDelay_{gamma_delay:.3g}.csv")
exportData((result_starvation_cycles/(oscillation_count)), directory, f"{today}_Chemostat_FlowScreen_{experiment_name}_StarvationCycles_GammaDelay_{gamma_delay:.3g}.csv")

exportData(result_typical_latency, directory, f"{today}_Chemostat_FlowScreen_{experiment_name}_LatencyT4_GammaDelay_{gamma_delay:.3g}.csv")
exportData(result_median_latency, directory, f"{today}_Chemostat_FlowScreen_{experiment_name}_LatencyT4_Median_GammaDelay_{gamma_delay:.3g}.csv")
exportData(result_substrate, directory, f"{today}_Chemostat_FlowScreen_{experiment_name}_SubstrateEnd_GammaDelay_{gamma_delay:.3g}.csv")
...
plt.savefig(f"Plots/{today}_Chemostat_FlowScreen_{experiment_name}_trend_norm_diff_GammaDelay_{gamma_delay:.3g}.svg", format = 'svg', dpi=600, bbox_inches='tight')
plt.show()
...
plt.savefig(f"Plots/{today}_Chemostat_FlowScreen_{experiment_name}_StarvationCycles_GammaDelay_{gamma_delay:.3g}.svg", format = 'svg', dpi=600, bbox_inches='tight')
plt.show()


plt.imshow(np.flipud(result_typical_latency), extent = [np.log10(flow_rates[0]), np.log10(flow_rates[-1]), (burst_T7_stat[0]), burst_T7_stat[-1]], aspect = 'auto', vmin = 0, vmax = 100, cmap = 'RdYlBu')
plt.colorbar()
plt.xlabel('log10(flow rate)')
plt.ylabel('burst size T7stat')
plt.title('Typical latency time T4')
plt.savefig(f"Plots/{today}_Chemostat_FlowScreen_{experiment_name}_TypicalLatency_GammaDelay_{gamma_delay:.3g}.svg", format = 'svg', dpi=600, bbox_inches='tight')
plt.show()

plt.imshow(np.flipud(result_median_latency), extent = [np.log10(flow_rates[0]), np.log10(flow_rates[-1]), (burst_T7_stat[0]), burst_T7_stat[-1]], aspect = 'auto', vmin = 0, vmax = 10, cmap = 'RdYlBu')
plt.colorbar()
plt.xlabel('log10(flow rate)')
plt.ylabel('burst size T7stat')
plt.title('Typical latency time T4 Median')
#plt.savefig(f"Plots/{today}_Chemostat_FlowScreen_{experiment_name}_MedianLatency_GammaDelay_{gamma_delay:.3g}.svg", format = 'svg', dpi=600, bbox_inches='tight')
plt.show()
